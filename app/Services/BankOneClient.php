...
class BankOneClient extends Client
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->endpoint = config('services.bone.base_url');
        $this->key = '';
        parent::__construct(array_merge([
            ...
            'query' => [
                'authToken' => config('services.bone.auth_token')
            ],
        ], $config));
    ...
    public function createCustomer(array $data): mixed
    {
        ...
        return $this->sends('POST', 'BankOneWebAPI/api/Customer/CreateCustomer/2', [
            'Content-Type' => 'application/json',
            'data' =>  $data
        ]);
    }
